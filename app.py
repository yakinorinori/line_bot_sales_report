...
class BotStateMachine:
    ...
    def step_0(self, event, user_message):
        if user_message == "売り上げ報告":
            ...
            user_data[event.source.user_id]['step'] = self.current_state
        else:
            line_bot_api.reply_message(
                event.reply_token,
    ...
    def step_1(self, event, user_message):
        user_data[event.source.user_id]['sales_info'].append({'date': user_message})
        line_bot_api.reply_message(
            event.reply_token,
    ...
    def step_2(self, event, user_message):
        cleaned_message = clean_input(user_message)
        if cleaned_message:
            try:
                receipt_count = int(cleaned_message)
                user_data[event.source.user_id]['sales_info'][0]['receipt_count'] = receipt_count
                user_data[event.source.user_id]['sales_info'][0]['transactions'] = []
                user_data[event.source.user_id]['current_receipt'] = 0  
                line_bot_api.reply_message(
                    event.reply_token,
    ...
    def step_3(self, event, user_message):
        ...
        if user_data[user_id]['current_receipt'] < user_data[user_id]['sales_info'][0]['receipt_count']:
            ...
            user_data[user_id]['step'] = self.current_state
        else:
            line_bot_api.reply_message(
                event.reply_token,
    ...
    def step_4(self, event, user_message):
        cleaned_message = clean_input(user_message)
        user_id = event.source.user_id
    ...
    def step_5(self, event, user_message):
        user_id = event.source.user_id
        sales = user_message
        current_receipt_index = user_data[user_id]['current_receipt'] - 1
...
def handle_message(event):
    ...
    # ステートマシンのインスタンス作成とメッセージ処理
    state_machine = BotStateMachine()
    state_machine.current_state = user_data[user_id]['step']  # 現在のステップを設定
    state_machine.states[state_machine.current_state](event, user_message)
